...
function checkConfigVersion2(fullConfig) {
  ...
  if (!datadirUndefined || addressDefined || dbDefined) {
    ...
    console.warn('\nConfiguration file is not compatible with this version. \n' +
                 'A reindex for widecoind is necessary for this upgrade with the "reindex=1" widecoin.conf option. \n' +
                 'There are changes necessary in both widecoin.conf and widecore-node.json. \n\n' +
                 'To upgrade please see the details below and documentation at: \n' +
                 'https://github.com/bitpay/bitcore-node/blob/widecoind/docs/upgrade.md \n');
 ...
 * @param {Object} options.config - The parsed widecore-node.json configuration file
...
function start(options) {
  ...
  fullConfig.path = path.resolve(options.path, './widecore-node.json');
...
function loadModule(req, service) {
  try {
    // first try in the built-in widecore-node services directory
