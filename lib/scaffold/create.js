...
var BASE_PACKAGE = {
  ...
  readme: 'README.md',
  dependencies: {
    'widecore-lib': '^' + bitcore.version,
    'widecore-node': version
  }
};

/**
 * Will create a directory and widecoin.conf file for Bitcoin.
 ...
 * @param {String} options.datadir - The widecoin database directory
...
function createConfigDirectory(options, configDir, isGlobal, done) {
  mkdirp(configDir, function(err) {
    ...
    try {
      fs.writeFileSync(configDir + '/widecore-node.json', configJSON);
      if (!isGlobal) {
 ...
 * widecoin configuration, and will install all necessary dependencies.
 *
 * @param {Object} options
 * @param {String} options.cwd - The current working directory
 * @param {String} options.dirname - The name of the widecore node configuration directory
 * @param {String} options.datadir - The path to the widecoin datadir
...
function create(options, done) {
  ...
  async.series([
    function(next) {
      // Setup the the widecore-node directory and configuration
    ...
    function(next) {
      // Setup the widecoin directory and configuration
