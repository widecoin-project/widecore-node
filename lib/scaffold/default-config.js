...
/**
 * Will return the path and default widecore-node configuration. It will search for the
 * configuration file in the "~/.widecoin" directory, and if it doesn't exist, it will create one
...
function getDefaultConfig(options) {
  ...
  var defaultPath = path.resolve(process.env.HOME, './.widecoin');
  var defaultConfigFile = path.resolve(defaultPath, './widecore-node.json');

  if (!fs.existsSync(defaultPath)) {
    mkdirp.sync(defaultPath);
  }

  var defaultServices = ['widecoind', 'web'];
