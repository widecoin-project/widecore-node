...
// can be modified to match the settings in widecoin.conf.
...
var widecoind = require('../').services.Bitcoin({
  node: {
    datadir: process.env.HOME + '/.widecoin',
    network: {
      name: 'testnet'
    }
  }
...
widecoind.on('ready', function() {

  console.log('Bitcoin Core ready');

  var client = new widecoin.Client({
    host: 'localhost',
    port: 18552,
    user: 'widecoin',
    pass: 'local321'
  ...
  async.series([
    function(next) {
      ...
      function widecoindGetBlockNative(deffered) {
        if (c >= hashesLength) {
          c = 0;
        }
      ...
      function widecoindGetBlockJsonRpc(deffered) {
        if (c >= hashesLength) {
          c = 0;
        }
        var hash = fixtureData.blockHashes[c];
      ...
      function widecoinGetTransactionNative(deffered) {
        if (c >= txLength) {
          c = 0;
        }
        var hash = fixtureData.txHashes[c];
        widecoind.getTransaction(hash, true, function(err, tx) {
          if (err) {
            throw err;
          }
          deffered.resolve();
      ...
      function widecoinGetTransactionJsonRpc(deffered) {
        if (c >= txLength) {
          c = 0;
        }
        var hash = fixtureData.txHashes[c];
      ...
      suite.add('widecoind getblock (native)', widecoindGetBlockNative, {
        defer: true,
        maxTime: maxTime
      });

      suite.add('widecoind getblock (json rpc)', widecoindGetBlockJsonRpc, {
        defer: true,
        maxTime: maxTime
      });

      suite.add('widecoind gettransaction (native)', widecoinGetTransactionNative, {
        defer: true,
        maxTime: maxTime
      });

      suite.add('widecoind gettransaction (json rpc)', widecoinGetTransactionJsonRpc, {
        defer: true,
        maxTime: maxTime
      });
