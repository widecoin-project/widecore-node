...
var readFileSync = sinon.stub().returns(fs.readFileSync(path.resolve(__dirname, '../data/widecoin.conf')));
var BitcoinService = proxyquire('../../lib/services/widecoind', {
  fs: {
    readFileSync: readFileSync
  }
});
var defaultBitcoinConf = fs.readFileSync(path.resolve(__dirname, '../data/default.widecoin.conf'), 'utf8');
...
describe('Bitcoin Service', function() {
  ...
  describe('@constructor', function() {
    it('will create an instance', function() {
      var widecoind = new BitcoinService(baseConfig);
      should.exist(widecoind);
    });
    it('will create an instance without `new`', function() {
      var widecoind = BitcoinService(baseConfig);
      should.exist(widecoind);
    });
    it('will init caches', function() {
      var widecoind = new BitcoinService(baseConfig);
      should.exist(widecoind.utxosCache);
      should.exist(widecoind.txidsCache);
      should.exist(widecoind.balanceCache);
      should.exist(widecoind.summaryCache);
      should.exist(widecoind.transactionDetailedCache);

      should.exist(widecoind.transactionCache);
      should.exist(widecoind.rawTransactionCache);
      should.exist(widecoind.blockCache);
      should.exist(widecoind.rawBlockCache);
      should.exist(widecoind.blockHeaderCache);
      should.exist(widecoind.zmqKnownTransactions);
      should.exist(widecoind.zmqKnownBlocks);
      should.exist(widecoind.lastTip);
      should.exist(widecoind.lastTipTimeout);
    });
    it('will init clients', function() {
      var widecoind = new BitcoinService(baseConfig);
      widecoind.nodes.should.deep.equal([]);
      widecoind.nodesIndex.should.equal(0);
      widecoind.nodes.push({client: sinon.stub()});
      should.exist(widecoind.client);
    });
    it('will set subscriptions', function() {
      var widecoind = new BitcoinService(baseConfig);
      widecoind.subscriptions.should.deep.equal({
        address: {},
        rawtransaction: [],
        hashblock: []
      });
  ...
  describe('#_initDefaults', function() {
    it('will set transaction concurrency', function() {
      var widecoind = new BitcoinService(baseConfig);
      widecoind._initDefaults({transactionConcurrency: 10});
      widecoind.transactionConcurrency.should.equal(10);
      widecoind._initDefaults({});
      widecoind.transactionConcurrency.should.equal(5);
    });
  });
  ...
  describe('#getAPIMethods', function() {
    it('will return spec', function() {
      var widecoind = new BitcoinService(baseConfig);
      var methods = widecoind.getAPIMethods();
  ...
  describe('#getPublishEvents', function() {
    it('will return spec', function() {
      var widecoind = new BitcoinService(baseConfig);
      var events = widecoind.getPublishEvents();
      should.exist(events);
      events.length.should.equal(3);
      events[0].name.should.equal('widecoind/rawtransaction');
      events[0].scope.should.equal(widecoind);
      events[0].subscribe.should.be.a('function');
      events[0].unsubscribe.should.be.a('function');
      events[1].name.should.equal('widecoind/hashblock');
      events[1].scope.should.equal(widecoind);
      events[1].subscribe.should.be.a('function');
      events[1].unsubscribe.should.be.a('function');
      events[2].name.should.equal('widecoind/addresstxid');
      events[2].scope.should.equal(widecoind);
    ...
    it('will call subscribe/unsubscribe with correct args', function() {
      var widecoind = new BitcoinService(baseConfig);
      widecoind.subscribe = sinon.stub();
      widecoind.unsubscribe = sinon.stub();
      var events = widecoind.getPublishEvents();

      events[0].subscribe('test');
      widecoind.subscribe.args[0][0].should.equal('rawtransaction');
      widecoind.subscribe.args[0][1].should.equal('test');

      events[0].unsubscribe('test');
      widecoind.unsubscribe.args[0][0].should.equal('rawtransaction');
      widecoind.unsubscribe.args[0][1].should.equal('test');

      events[1].subscribe('test');
      widecoind.subscribe.args[1][0].should.equal('hashblock');
      widecoind.subscribe.args[1][1].should.equal('test');

      events[1].unsubscribe('test');
      widecoind.unsubscribe.args[1][0].should.equal('hashblock');
      widecoind.unsubscribe.args[1][1].should.equal('test');
    });
  });
  ...
  describe('#subscribe', function() {
    ...
    it('will push to subscriptions', function() {
      var widecoind = new BitcoinService(baseConfig);
      var emitter = {};
      widecoind.subscribe('hashblock', emitter);
      widecoind.subscriptions.hashblock[0].should.equal(emitter);

      var emitter2 = {};
      widecoind.subscribe('rawtransaction', emitter2);
      widecoind.subscriptions.rawtransaction[0].should.equal(emitter2);
    });
  });
  ...
  describe('#unsubscribe', function() {
    ...
    it('will remove item from subscriptions', function() {
      ...
      var emitter4 = {};
      var emitter5 = {};
      widecoind.subscribe('hashblock', emitter1);
      widecoind.subscribe('hashblock', emitter2);
      widecoind.subscribe('hashblock', emitter3);
      widecoind.subscribe('hashblock', emitter4);
      widecoind.subscribe('hashblock', emitter5);
      widecoind.subscriptions.hashblock.length.should.equal(5);

      widecoind.unsubscribe('hashblock', emitter3);
      widecoind.subscriptions.hashblock.length.should.equal(4);
      widecoind.subscriptions.hashblock[0].should.equal(emitter1);
      widecoind.subscriptions.hashblock[1].should.equal(emitter2);
      widecoind.subscriptions.hashblock[2].should.equal(emitter4);
      widecoind.subscriptions.hashblock[3].should.equal(emitter5);
    });
    it('will not remove item an already unsubscribed item', function() {
      var widecoind = new BitcoinService(baseConfig);
      var emitter1 = {};
      var emitter3 = {};
      widecoind.subscriptions.hashblock= [emitter1];
      widecoind.unsubscribe('hashblock', emitter3);
      widecoind.subscriptions.hashblock.length.should.equal(1);
      widecoind.subscriptions.hashblock[0].should.equal(emitter1);
    });
  });
  ...
  describe('#subscribeAddress', function() {
    ...
    it('will not an invalid address', function() {
      ...
      widecoind.subscribeAddress(emitter, ['invalidaddress']);
      should.not.exist(widecoind.subscriptions.address['invalidaddress']);
    });
    it('will add a valid address', function() {
      var widecoind = new BitcoinService(baseConfig);
      var emitter = new EventEmitter();
      widecoind.subscribeAddress(emitter, ['2N2JD6wb56AfK4tfmM6PwdVmoYk2dCKf4Br']);
      should.exist(widecoind.subscriptions.address['2N2JD6wb56AfK4tfmM6PwdVmoYk2dCKf4Br']);
    });
    it('will handle multiple address subscribers', function() {
      var widecoind = new BitcoinService(baseConfig);
      var emitter1 = new EventEmitter();
      var emitter2 = new EventEmitter();
      widecoind.subscribeAddress(emitter1, ['2N2JD6wb56AfK4tfmM6PwdVmoYk2dCKf4Br']);
      widecoind.subscribeAddress(emitter2, ['2N2JD6wb56AfK4tfmM6PwdVmoYk2dCKf4Br']);
      should.exist(widecoind.subscriptions.address['2N2JD6wb56AfK4tfmM6PwdVmoYk2dCKf4Br']);
      widecoind.subscriptions.address['2N2JD6wb56AfK4tfmM6PwdVmoYk2dCKf4Br'].length.should.equal(2);
    });
    it('will not add the same emitter twice', function() {
      ...
      widecoind.subscribeAddress(emitter1, ['2N2JD6wb56AfK4tfmM6PwdVmoYk2dCKf4Br']);
      should.exist(widecoind.subscriptions.address['2N2JD6wb56AfK4tfmM6PwdVmoYk2dCKf4Br']);
      widecoind.subscriptions.address['2N2JD6wb56AfK4tfmM6PwdVmoYk2dCKf4Br'].length.should.equal(1);
    });
  });
  ...
  describe('#unsubscribeAddress', function() {
    ...
    it('it will remove a subscription', function() {
      ...
      should.exist(widecoind.subscriptions.address['2N2JD6wb56AfK4tfmM6PwdVmoYk2dCKf4Br']);
      widecoind.subscriptions.address['2N2JD6wb56AfK4tfmM6PwdVmoYk2dCKf4Br'].length.should.equal(2);
      widecoind.unsubscribeAddress(emitter1, ['2N2JD6wb56AfK4tfmM6PwdVmoYk2dCKf4Br']);
      widecoind.subscriptions.address['2N2JD6wb56AfK4tfmM6PwdVmoYk2dCKf4Br'].length.should.equal(1);
    });
    it('will unsubscribe subscriptions for an emitter', function() {
      ...
      var emitter1 = new EventEmitter();
      var emitter2 = new EventEmitter();
      widecoind.subscriptions.address['2N2JD6wb56AfK4tfmM6PwdVmoYk2dCKf4Br'] = [emitter1, emitter2];
      widecoind.unsubscribeAddress(emitter1);
      widecoind.subscriptions.address['2N2JD6wb56AfK4tfmM6PwdVmoYk2dCKf4Br'].length.should.equal(1);
    });
    it('will NOT unsubscribe subscription with missing address', function() {
      ...
      var emitter2 = new EventEmitter();
      widecoind.subscriptions.address['2N2JD6wb56AfK4tfmM6PwdVmoYk2dCKf4Br'] = [emitter1, emitter2];
      widecoind.unsubscribeAddress(emitter1, ['LWx1jmpcMAQMXou6pbLyfMU7zETnbhy3bo']);
      widecoind.subscriptions.address['2N2JD6wb56AfK4tfmM6PwdVmoYk2dCKf4Br'].length.should.equal(2);
    });
    it('will NOT unsubscribe subscription with missing emitter', function() {
      ...
      var emitter1 = new EventEmitter();
      var emitter2 = new EventEmitter();
      widecoind.subscriptions.address['2N2JD6wb56AfK4tfmM6PwdVmoYk2dCKf4Br'] = [emitter2];
      widecoind.unsubscribeAddress(emitter1, ['2N2JD6wb56AfK4tfmM6PwdVmoYk2dCKf4Br']);
      widecoind.subscriptions.address['2N2JD6wb56AfK4tfmM6PwdVmoYk2dCKf4Br'].length.should.equal(1);
      widecoind.subscriptions.address['2N2JD6wb56AfK4tfmM6PwdVmoYk2dCKf4Br'][0].should.equal(emitter2);
    });
    it('will remove empty addresses', function() {
      ...
      widecoind.subscriptions.address['2N2JD6wb56AfK4tfmM6PwdVmoYk2dCKf4Br'] = [emitter1, emitter2];
      widecoind.unsubscribeAddress(emitter1, ['2N2JD6wb56AfK4tfmM6PwdVmoYk2dCKf4Br']);
      widecoind.unsubscribeAddress(emitter2, ['2N2JD6wb56AfK4tfmM6PwdVmoYk2dCKf4Br']);
      should.not.exist(widecoind.subscriptions.address['2N2JD6wb56AfK4tfmM6PwdVmoYk2dCKf4Br']);
    });
    it('will unsubscribe emitter for all addresses', function() {
      ...
      var emitter2 = new EventEmitter();
      widecoind.subscriptions.address['2N2JD6wb56AfK4tfmM6PwdVmoYk2dCKf4Br'] = [emitter1, emitter2];
      widecoind.subscriptions.address['LWx1jmpcMAQMXou6pbLyfMU7zETnbhy3bo'] = [emitter1, emitter2];
      sinon.spy(widecoind, 'unsubscribeAddressAll');
      widecoind.unsubscribeAddress(emitter1);
      widecoind.unsubscribeAddressAll.callCount.should.equal(1);
      widecoind.subscriptions.address['2N2JD6wb56AfK4tfmM6PwdVmoYk2dCKf4Br'].length.should.equal(1);
      widecoind.subscriptions.address['LWx1jmpcMAQMXou6pbLyfMU7zETnbhy3bo'].length.should.equal(1);
    });
  });
  ...
  describe('#unsubscribeAddressAll', function() {
    ...
    it('will unsubscribe emitter for all addresses', function() {
      ...
      widecoind.subscriptions.address['2N2JD6wb56AfK4tfmM6PwdVmoYk2dCKf4Br'] = [emitter1, emitter2];
      widecoind.subscriptions.address['LWx1jmpcMAQMXou6pbLyfMU7zETnbhy3bo'] = [emitter1, emitter2];
      widecoind.subscriptions.address['mgY65WSfEmsyYaYPQaXhmXMeBhwp4EcsQW'] = [emitter2];
      widecoind.subscriptions.address['3CMNFxN1oHBc4R1EpboAL5yzHGgE611Xou'] = [emitter1];
      widecoind.unsubscribeAddress(emitter1);
      widecoind.subscriptions.address['2N2JD6wb56AfK4tfmM6PwdVmoYk2dCKf4Br'].length.should.equal(1);
      widecoind.subscriptions.address['LWx1jmpcMAQMXou6pbLyfMU7zETnbhy3bo'].length.should.equal(1);
      widecoind.subscriptions.address['mgY65WSfEmsyYaYPQaXhmXMeBhwp4EcsQW'].length.should.equal(1);
      should.not.exist(widecoind.subscriptions.address['3CMNFxN1oHBc4R1EpboAL5yzHGgE611Xou']);
    });
  });
  ...
  describe('#_getDefaultConfig', function() {
    it('will generate config file from defaults', function() {
      var widecoind = new BitcoinService(baseConfig);
      var config = widecoind._getDefaultConfig();
  ...
  describe('#_loadSpawnConfiguration', function() {
    ...
    it('will parse a widecoin.conf file', function() {
      var TestBitcoin = proxyquire('../../lib/services/widecoind', {
        fs: {
          readFileSync: readFileSync,
      ...
      var widecoind = new TestBitcoin(baseConfig);
      widecoind.options.spawn.datadir = '/tmp/.widecoin';
      var node = {};
      widecoind._loadSpawnConfiguration(node);
      should.exist(widecoind.spawn.config);
      widecoind.spawn.config.should.deep.equal({
        addressindex: 1,
        checkblocks: 144,
        dbcache: 8192,
    ...
    it('will expand relative datadir to absolute path', function() {
      ...
      var widecoind = new TestBitcoin(config);
      widecoind.options.spawn.datadir = './data';
      var node = {};
      widecoind._loadSpawnConfiguration(node);
      widecoind.options.spawn.datadir.should.equal('/tmp/.bitcore/data');
    });
    it('should throw an exception if txindex isn\'t enabled in the configuration', function() {
      var TestBitcoin = proxyquire('../../lib/services/widecoind', {
        fs: {
          readFileSync: sinon.stub().returns(fs.readFileSync(__dirname + '/../data/badwidecoin.conf')),
          existsSync: sinon.stub().returns(true),
        },
        mkdirp: {
          sync: sinon.stub()
      ...
      var widecoind = new TestBitcoin(baseConfig);
      (function() {
        widecoind._loadSpawnConfiguration({datadir: './test'});
  ...
  describe('#_checkConfigIndexes', function() {
    ...
    it('should warn the user if reindex is set to 1 in the widecoin.conf file', function() {
      var widecoind = new BitcoinService(baseConfig);
      var config = {
        txindex: 1,
        addressindex: 1,
      ...
      var node = {};
      widecoind._checkConfigIndexes(config, node);
  ...
  describe('#_resetCaches', function() {
    it('will reset LRU caches', function() {
      ...
      for (var i = 0; i < 10; i++) {
        ...
        widecoind.transactionDetailedCache.set(keys[i], {});
        widecoind.utxosCache.set(keys[i], {});
        widecoind.txidsCache.set(keys[i], {});
        widecoind.balanceCache.set(keys[i], {});
        widecoind.summaryCache.set(keys[i], {});
      }
      widecoind._resetCaches();
      should.equal(widecoind.transactionDetailedCache.get(keys[0]), undefined);
      should.equal(widecoind.utxosCache.get(keys[0]), undefined);
      should.equal(widecoind.txidsCache.get(keys[0]), undefined);
      should.equal(widecoind.balanceCache.get(keys[0]), undefined);
      should.equal(widecoind.summaryCache.get(keys[0]), undefined);
    });
  });
  ...
  describe('#_tryAllClients', function() {
    it('will retry for each node client', function(done) {
      var widecoind = new BitcoinService(baseConfig);
      widecoind.tryAllInterval = 1;
      widecoind.nodes.push({
        client: {
      ...
      widecoind._tryAllClients(function(client, next) {
        client.getInfo(next);
      }, function(err) {
        if (err) {
          return done(err);
        }
        widecoind.nodes[0].client.getInfo.callCount.should.equal(1);
        widecoind.nodes[1].client.getInfo.callCount.should.equal(1);
        widecoind.nodes[2].client.getInfo.callCount.should.equal(1);
        done();
      });
    });
    it('will start using the current node index (round-robin)', function(done) {
      ...
      widecoind._tryAllClients(function(client, next) {
        ...
        widecoind.nodes[1].client.getInfo.callCount.should.equal(1);
        widecoind.nodes[2].client.getInfo.callCount.should.equal(1);
        widecoind.nodesIndex.should.equal(2);
        done();
      });
    });
  ...
  describe('#_wrapRPCError', function() {
    it('will convert widecoind-rpc error object into JavaScript error', function() {
      var widecoind = new BitcoinService(baseConfig);
      var error = widecoind._wrapRPCError({message: 'Test error', code: -1});
  ...
  describe('#_initChain', function() {
    ...
    it('will set height and genesis buffer', function(done) {
      ...
      widecoind.getRawBlock = sinon.stub().callsArgWith(1, null, genesisBuffer);
      widecoind.nodes.push({
        client: {
      ...
      widecoind._initChain(function() {
        log.info.callCount.should.equal(1);
        widecoind.getRawBlock.callCount.should.equal(1);
        widecoind.getRawBlock.args[0][0].should.equal('genesishash');
        widecoind.height.should.equal(5000);
        widecoind.genesisBuffer.should.equal(genesisBuffer);
        done();
      });
    });
    it('it will handle error from getBestBlockHash', function(done) {
      ...
      widecoind._initChain(function(err) {
        err.should.be.instanceOf(Error);
        done();
      });
    });
    ...
    it('it will handle error from getRawBlock', function(done) {
      ...
      widecoind.getRawBlock = sinon.stub().callsArgWith(1, new Error('test'));
      widecoind._initChain(function(err) {
        err.should.be.instanceOf(Error);
        done();
      });
  ...
  describe('#_getDefaultConf', function() {
    ...
    it('will get default rpc port for livenet', function() {
      ...
      var widecoind = new BitcoinService(config);
      widecoind._getDefaultConf().rpcport.should.equal(9332);
    });
    it('will get default rpc port for testnet', function() {
      ...
      var widecoind = new BitcoinService(config);
      widecoind._getDefaultConf().rpcport.should.equal(19332);
    });
  ...
  describe('#_getNetworkConfigPath', function() {
    ...
    it('will get default config path for livenet', function() {
      ...
      var widecoind = new BitcoinService(config);
      should.equal(widecoind._getNetworkConfigPath(), undefined);
    });
    it('will get default rpc port for testnet', function() {
      var config = {
        node: {
      ...
      var widecoind = new BitcoinService(config);
      widecoind._getNetworkConfigPath().should.equal('testnet3/widecoin.conf');
    });
    it('will get default rpc port for regtest', function() {
      bitcore.Networks.enableRegtest();
      var config = {
      ...
      var widecoind = new BitcoinService(config);
      widecoind._getNetworkConfigPath().should.equal('regtest/widecoin.conf');
    });
  });
  ...
  describe('#_getNetworkOption', function() {
    ...
    it('return --testnet for testnet', function() {
      var widecoind = new BitcoinService(baseConfig);
      widecoind.node.network = bitcore.Networks.testnet;
      widecoind._getNetworkOption().should.equal('--testnet');
    });
    it('return --regtest for testnet', function() {
      var widecoind = new BitcoinService(baseConfig);
      widecoind.node.network = bitcore.Networks.testnet;
      bitcore.Networks.enableRegtest();
      widecoind._getNetworkOption().should.equal('--regtest');
    });
    it('return undefined for livenet', function() {
      var widecoind = new BitcoinService(baseConfig);
      widecoind.node.network = bitcore.Networks.livenet;
      bitcore.Networks.enableRegtest();
      should.equal(widecoind._getNetworkOption(), undefined);
    });
  });
  ...
  describe('#_zmqBlockHandler', function() {
    it('will emit block', function(done) {
      ...
      widecoind._rapidProtectedUpdateTip = sinon.stub();
      widecoind.on('block', function(block) {
        block.should.equal(message);
        done();
      });
      widecoind._zmqBlockHandler(node, message);
    });
    ...
    it('will call function to update tip', function() {
      ...
      widecoind._rapidProtectedUpdateTip = sinon.stub();
      widecoind._zmqBlockHandler(node, message);
      widecoind._rapidProtectedUpdateTip.callCount.should.equal(1);
      widecoind._rapidProtectedUpdateTip.args[0][0].should.equal(node);
      widecoind._rapidProtectedUpdateTip.args[0][1].should.equal(message);
    });
    it('will emit to subscribers', function(done) {
      ...
      widecoind._rapidProtectedUpdateTip = sinon.stub();
      var emitter = new EventEmitter();
      widecoind.subscriptions.hashblock.push(emitter);
      emitter.on('widecoind/hashblock', function(blockHash) {
  ...
  describe('#_rapidProtectedUpdateTip', function() {
    it('will limit tip updates with rapid calls', function(done) {
      ...
      var count = 0;
      function repeat() {
        widecoind._rapidProtectedUpdateTip(node, message);
        count++;
        if (count < 50) {
          repeat();
        }
  ...
  describe('#_updateTip', function() {
    ...
    it('log and emit rpc error from get block', function(done) {
      var widecoind = new BitcoinService(baseConfig);
      widecoind.syncPercentage = sinon.stub();
      widecoind.on('error', function(err) {
      ...
      widecoind._updateTip(node, message);
    });
    it('emit synced if percentage is 100', function(done) {
      var widecoind = new BitcoinService(baseConfig);
      widecoind.syncPercentage = sinon.stub().callsArgWith(0, null, 100);
      widecoind.on('synced', function() {
        done();
      });
      var node = {
        client: {
    ...
    it('NOT emit synced if percentage is less than 100', function(done) {
      var widecoind = new BitcoinService(baseConfig);
      widecoind.syncPercentage = sinon.stub().callsArgWith(0, null, 99);
      widecoind.on('synced', function() {
    ...
    it('log and emit error from syncPercentage', function(done) {
      var widecoind = new BitcoinService(baseConfig);
      widecoind.syncPercentage = sinon.stub().callsArgWith(0, new Error('test'));
      widecoind.on('error', function(err) {
        log.error.callCount.should.equal(1);
        err.message.should.equal('test');
        done();
    ...
    it('reset caches and set height', function(done) {
      var widecoind = new BitcoinService(baseConfig);
      widecoind.syncPercentage = sinon.stub();
      widecoind._resetCaches = sinon.stub();
      widecoind.on('tip', function(height) {
        widecoind._resetCaches.callCount.should.equal(1);
        height.should.equal(10);
        widecoind.height.should.equal(10);
        done();
      });
      var node = {
        client: {
    ...
    it('will not call syncPercentage if node is stopping', function(done) {
      ...
      widecoind.on('tip', function() {
        widecoind.syncPercentage.callCount.should.equal(0);
        done();
      });
      widecoind._updateTip(node, message);
    });
  ...
  describe('#_getAddressesFromTransaction', function() {
    it('will get results using bitcore.Transaction', function() {
      ...
      tx.sign(privkey);
      var addresses = widecoind._getAddressesFromTransaction(tx);
  ...
  describe('#_notifyAddressTxidSubscribers', function() {
    it('will emit event if matching addresses', function(done) {
      ...
      widecoind._getAddressesFromTransaction = sinon.stub().returns([address]);
      var emitter = new EventEmitter();
      widecoind.subscriptions.address[address] = [emitter];
      var txid = '46f24e0c274fc07708b781963576c4c5d5625d926dbb0a17fa865dcd9fe58ea0';
      var transaction = {};
      emitter.on('widecoind/addresstxid', function(data) {
        data.address.should.equal(address);
        data.txid.should.equal(txid);
        done();
      });
      sinon.spy(emitter, 'emit');
      widecoind._notifyAddressTxidSubscribers(txid, transaction);
  ...
  describe('#_zmqTransactionHandler', function() {
    it('will emit to subscribers', function(done) {
      ...
      var emitter = new EventEmitter();
      widecoind.subscriptions.rawtransaction.push(emitter);
      emitter.on('widecoind/rawtransaction', function(hex) {
        hex.should.be.a('string');
        hex.should.equal(expectedBuffer.toString('hex'));
        done();
      });
      var node = {};
      widecoind._zmqTransactionHandler(node, expectedBuffer);
    });
    it('will NOT emit to subscribers more than once for the same tx', function(done) {
      ...
      var emitter = new EventEmitter();
      widecoind.subscriptions.rawtransaction.push(emitter);
      emitter.on('widecoind/rawtransaction', function() {
        done();
      });
      var node = {};
      widecoind._zmqTransactionHandler(node, expectedBuffer);
    ...
    it('will emit "tx" event', function(done) {
      var widecoind = new BitcoinService(baseConfig);
      var expectedBuffer = new Buffer(txhex, 'hex');
      widecoind.on('tx', function(buffer) {
  ...
  describe('#_checkSyncedAndSubscribeZmqEvents', function() {
    ...
    it('log errors, update tip and subscribe to zmq events', function(done) {
      var widecoind = new BitcoinService(baseConfig);
      widecoind._updateTip = sinon.stub();
      widecoind._subscribeZmqEvents = sinon.stub();
      var blockEvents = 0;
      widecoind.on('block', function() {
        blockEvents++;
      });
      var getBestBlockHash = sinon.stub().callsArgWith(0, null, {
      ...
      widecoind._checkSyncedAndSubscribeZmqEvents(node);
      setTimeout(function() {
        log.error.callCount.should.equal(2);
        blockEvents.should.equal(11);
        widecoind._updateTip.callCount.should.equal(11);
        widecoind._subscribeZmqEvents.callCount.should.equal(1);
        done();
      }, 200);
    });
  ...
  describe('#_subscribeZmqEvents', function() {
    it('will call subscribe on zmq socket', function() {
      ...
      widecoind._subscribeZmqEvents(node);
    ...
    it('will call relevant handler for rawtx topics', function(done) {
      var widecoind = new BitcoinService(baseConfig);
      widecoind._zmqTransactionHandler = sinon.stub();
      var node = {
      ...
      node.zmqSubSocket.on('message', function() {
        widecoind._zmqTransactionHandler.callCount.should.equal(1);
        done();
      });
    ...
    it('will call relevant handler for hashblock topics', function(done) {
      var widecoind = new BitcoinService(baseConfig);
      widecoind._zmqBlockHandler = sinon.stub();
      var node = {
        zmqSubSocket: new EventEmitter()
      };
      node.zmqSubSocket.subscribe = sinon.stub();
      widecoind._subscribeZmqEvents(node);
      node.zmqSubSocket.on('message', function() {
        widecoind._zmqBlockHandler.callCount.should.equal(1);
        done();
      });
    ...
    it('will ignore unknown topic types', function(done) {
      ...
      widecoind._subscribeZmqEvents(node);
      node.zmqSubSocket.on('message', function() {
        widecoind._zmqBlockHandler.callCount.should.equal(0);
        widecoind._zmqTransactionHandler.callCount.should.equal(0);
        done();
      });
  ...
  describe('#_initZmqSubSocket', function() {
    it('will setup zmq socket', function() {
      ...
      var widecoind = new BitcoinService(baseConfig);
      var node = {};
      widecoind._initZmqSubSocket(node, 'url');
  ...
  describe('#_checkReindex', function() {
    ...
    it('give error from client getblockchaininfo', function(done) {
      ...
      widecoind._checkReindex(node, function(err) {
        should.exist(err);
    ...
    it('will wait until sync is 100 percent', function(done) {
      ...
      widecoind._checkReindex(node, function() {
  ...
  describe('#_loadTipFromNode', function() {
    ...
    it('will give rpc from client getbestblockhash', function(done) {
      ...
      widecoind._loadTipFromNode(node, function(err) {
        err.should.be.instanceof(Error);
    ...
    it('will set height and emit tip', function(done) {
      ...
      widecoind.on('tip', function(height) {
        ...
        widecoind.height.should.equal(100);
        done();
      });
      widecoind._loadTipFromNode(node, function(err) {
        if (err) {
  ...
  describe('#_stopSpawnedProcess', function() {
    ...
    it('it will kill process and resume', function(done) {
      ...
      var TestBitcoinService = proxyquire('../../lib/services/widecoind', {
        fs: {
          readFile: readFile
        }
      });
      var widecoind = new TestBitcoinService(baseConfig);
      widecoind.spawnStopTime = 1;
      widecoind._process = {};
      widecoind._process.kill = sinon.stub();
      widecoind._stopSpawnedBitcoin(function(err) {
        if (err) {
          return done(err);
        }
        widecoind._process.kill.callCount.should.equal(1);
        log.warn.callCount.should.equal(1);
        done();
      });
    });
    it('it will attempt to kill process and resume', function(done) {
      ...
      error2.code = 'ESRCH';
      widecoind._process.kill = sinon.stub().throws(error2);
      widecoind._stopSpawnedBitcoin(function(err) {
        if (err) {
          return done(err);
        }
  ...
  describe('#_spawnChildProcess', function() {
    ...
    it('will give error from spawn config', function(done) {
      var widecoind = new BitcoinService(baseConfig);
      widecoind._loadSpawnConfiguration = sinon.stub();
      widecoind._loadSpawnConfiguration = sinon.stub().throws(new Error('test'));
      widecoind._spawnChildProcess(function(err) {
        err.should.be.instanceof(Error);
        err.message.should.equal('test');
        done();
      });
    ...
    it('will give error from stopSpawnedBitcoin', function() {
      var widecoind = new BitcoinService(baseConfig);
      widecoind._loadSpawnConfiguration = sinon.stub();
      widecoind._stopSpawnedBitcoin = sinon.stub().callsArgWith(0, new Error('test'));
      widecoind._spawnChildProcess(function(err) {
        err.should.be.instanceOf(Error);
        err.message.should.equal('test');
      });
    ...
    it('will exit spawn if shutdown', function() {
      ...
      var widecoind = new TestBitcoinService(config);
      widecoind.spawn = {};
      widecoind._loadSpawnConfiguration = sinon.stub();
      widecoind._stopSpawnedBitcoin = sinon.stub().callsArgWith(0, null);
      widecoind.node.stopping = true;
      widecoind._spawnChildProcess(function(err) {
        err.should.be.instanceOf(Error);
    ...
    it('will include network with spawn command and init zmq/rpc on node', function(done) {
      ...
      widecoind._loadSpawnConfiguration = sinon.stub();
      widecoind.spawn = {};
      widecoind.spawn.exec = 'testexec';
      widecoind.spawn.configPath = 'testdir/widecoin.conf';
      widecoind.spawn.datadir = 'testdir';
      widecoind.spawn.config = {};
      widecoind.spawn.config.rpcport = 20001;
      widecoind.spawn.config.rpcuser = 'widecoin';
      widecoind.spawn.config.rpcpassword = 'password';
      widecoind.spawn.config.zmqpubrawtx = 'tcp://127.0.0.1:30001';

      widecoind._loadTipFromNode = sinon.stub().callsArgWith(1, null);
      widecoind._initZmqSubSocket = sinon.stub();
      widecoind._checkSyncedAndSubscribeZmqEvents = sinon.stub();
      widecoind._checkReindex = sinon.stub().callsArgWith(1, null);
      widecoind._spawnChildProcess(function(err, node) {
        should.not.exist(err);
        spawn.callCount.should.equal(1);
        spawn.args[0][0].should.equal('testexec');
        spawn.args[0][1].should.deep.equal([
          '--conf=testdir/widecoin.conf',
          '--datadir=testdir',
          '--testnet'
        ]);
        ...
        widecoind._loadTipFromNode.callCount.should.equal(1);
        widecoind._initZmqSubSocket.callCount.should.equal(1);
        should.exist(widecoind._initZmqSubSocket.args[0][0].client);
        widecoind._initZmqSubSocket.args[0][1].should.equal('tcp://127.0.0.1:30001');
        widecoind._checkSyncedAndSubscribeZmqEvents.callCount.should.equal(1);
        should.exist(widecoind._checkSyncedAndSubscribeZmqEvents.args[0][0].client);
        should.exist(node);
        should.exist(node.client);
        done();
      });
    });
    it('will respawn widecoind spawned process', function(done) {
      var process = new EventEmitter();
      var spawn = sinon.stub().returns(process);
      var TestBitcoinService = proxyquire('../../lib/services/widecoind', {
        fs: {
      ...
      widecoind._loadSpawnConfiguration = sinon.stub();
      widecoind.spawn = {};
      widecoind.spawn.exec = 'widecoind';
      widecoind.spawn.datadir = '/tmp/widecoin';
      widecoind.spawn.configPath = '/tmp/widecoin/widecoin.conf';
      widecoind.spawn.config = {};
      widecoind.spawnRestartTime = 1;
      widecoind._loadTipFromNode = sinon.stub().callsArg(1);
      widecoind._initZmqSubSocket = sinon.stub();
      widecoind._checkReindex = sinon.stub().callsArg(1);
      widecoind._checkSyncedAndSubscribeZmqEvents = sinon.stub();
      widecoind._stopSpawnedBitcoin = sinon.stub().callsArg(0);
      sinon.spy(widecoind, '_spawnChildProcess');
      widecoind._spawnChildProcess(function(err) {
        if (err) {
          return done(err);
        }
        process.once('exit', function() {
          setTimeout(function() {
            widecoind._spawnChildProcess.callCount.should.equal(2);
            done();
          }, 5);
        });
        process.emit('exit', 1);
    ...
    it('will emit error during respawn', function(done) {
      ...
      widecoind._spawnChildProcess(function(err) {
        ...
        widecoind._spawnChildProcess = sinon.stub().callsArgWith(0, new Error('test'));
        widecoind.on('error', function(err) {
          err.should.be.instanceOf(Error);
          err.message.should.equal('test');
          done();
    ...
    it('will NOT respawn widecoind spawned process if shutting down', function(done) {
      var process = new EventEmitter();
      var spawn = sinon.stub().returns(process);
      var TestBitcoinService = proxyquire('../../lib/services/widecoind', {
        fs: {
      ...
      widecoind._spawnChildProcess(function(err) {
        ...
        process.once('exit', function() {
          setTimeout(function() {
            widecoind._spawnChildProcess.callCount.should.equal(1);
            done();
          }, 5);
        });
        process.emit('exit', 1);
    ...
    it('will give error after 60 retries', function(done) {
      ...
      var widecoind = new TestBitcoinService(baseConfig);
      widecoind.startRetryInterval = 1;
      widecoind._loadSpawnConfiguration = sinon.stub();
      widecoind.spawn = {};
      widecoind.spawn.exec = 'testexec';
      widecoind.spawn.configPath = 'testdir/widecoin.conf';
      ...
      widecoind.spawn.config.rpcpassword = 'password';
      widecoind.spawn.config.zmqpubrawtx = 'tcp://127.0.0.1:30001';
      widecoind._loadTipFromNode = sinon.stub().callsArgWith(1, new Error('test'));
      widecoind._spawnChildProcess(function(err) {
        widecoind._loadTipFromNode.callCount.should.equal(60);
        err.should.be.instanceof(Error);
        done();
      });
    });
    it('will give error from check reindex', function(done) {
      ...
      widecoind._initZmqSubSocket = sinon.stub();
      widecoind._checkSyncedAndSubscribeZmqEvents = sinon.stub();
      widecoind._checkReindex = sinon.stub().callsArgWith(1, new Error('test'));
  ...
  describe('#_connectProcess', function() {
    it('will give error if connecting while shutting down', function(done) {
      ...
      var widecoind = new BitcoinService(config);
      widecoind.node.stopping = true;
      widecoind.startRetryInterval = 100;
      widecoind._loadTipFromNode = sinon.stub();
      widecoind._connectProcess({}, function(err) {
        err.should.be.instanceof(Error);
        err.message.should.match(/Stopping while trying to connect/);
        widecoind._loadTipFromNode.callCount.should.equal(0);
        done();
      });
    });
    it('will give error from loadTipFromNode after 60 retries', function(done) {
      ...
      widecoind.startRetryInterval = 1;
      var config = {};
      widecoind._connectProcess(config, function(err) {
        err.should.be.instanceof(Error);
        widecoind._loadTipFromNode.callCount.should.equal(60);
        done();
      });
    ...
    it('will init zmq/rpc on node', function(done) {
      ...
      widecoind._loadTipFromNode = sinon.stub().callsArgWith(1, null);
      var config = {};
      widecoind._connectProcess(config, function(err, node) {
        should.not.exist(err);
        widecoind._loadTipFromNode.callCount.should.equal(1);
        widecoind._initZmqSubSocket.callCount.should.equal(1);
        widecoind._loadTipFromNode.callCount.should.equal(1);
  ...
  describe('#start', function() {
    ...
    it('will give error from connectProcess', function(done) {
      var widecoind = new BitcoinService(baseConfig);
      widecoind._connectProcess = sinon.stub().callsArgWith(1, new Error('test'));
      widecoind.options = {
        connect: [
          {}
        ]
      ...
      widecoind.start(function(err) {
        widecoind._connectProcess.callCount.should.equal(1);
        err.should.be.instanceof(Error);
        err.message.should.equal('test');
        done();
      });
    ...
    it('will push node from spawnChildProcess', function(done) {
      var widecoind = new BitcoinService(baseConfig);
      var node = {};
      widecoind._initChain = sinon.stub().callsArg(0);
      widecoind._spawnChildProcess = sinon.stub().callsArgWith(0, null, node);
      widecoind.options = {
        spawn: {}
      };
      widecoind.start(function(err) {
        should.not.exist(err);
        widecoind.nodes.length.should.equal(1);
        done();
      });
    });
    it('will push node from connectProcess', function(done) {
      ...
      widecoind._initChain = sinon.stub().callsArg(0);
      var nodes = [{}];
      widecoind._connectProcess = sinon.stub().callsArgWith(1, null, nodes);
      widecoind.options = {
        connect: [
          {}
        ]
  ...
  describe('#isSynced', function() {
    it('will give error from syncPercentage', function(done) {
      var widecoind = new BitcoinService(baseConfig);
      widecoind.syncPercentage = sinon.stub().callsArgWith(0, new Error('test'));
      widecoind.isSynced(function(err) {
        should.exist(err);
        err.message.should.equal('test');
        done();
      });
    ...
    it('will give "true" if percentage is 100.00', function(done) {
      var widecoind = new BitcoinService(baseConfig);
      widecoind.syncPercentage = sinon.stub().callsArgWith(0, null, 100.00);
      widecoind.isSynced(function(err, synced) {
        if (err) {
          return done(err);
        }
    ...
    it('will give "true" if percentage is 99.98', function(done) {
      var widecoind = new BitcoinService(baseConfig);
      widecoind.syncPercentage = sinon.stub().callsArgWith(0, null, 99.98);
      widecoind.isSynced(function(err, synced) {
        if (err) {
          return done(err);
        }
    ...
    it('will give "false" if percentage is 99.49', function(done) {
      var widecoind = new BitcoinService(baseConfig);
      widecoind.syncPercentage = sinon.stub().callsArgWith(0, null, 99.49);
      widecoind.isSynced(function(err, synced) {
        if (err) {
          return done(err);
        }
    ...
    it('will give "false" if percentage is 1', function(done) {
      var widecoind = new BitcoinService(baseConfig);
      widecoind.syncPercentage = sinon.stub().callsArgWith(0, null, 1);
      widecoind.isSynced(function(err, synced) {
        if (err) {
          return done(err);
        }
  ...
  describe('#syncPercentage', function() {
    it('will give rpc error', function(done) {
      ...
      widecoind.syncPercentage(function(err) {
        should.exist(err);
    ...
    it('will call client getInfo and give result', function(done) {
      ...
      widecoind.syncPercentage(function(err, percentage) {
        if (err) {
          return done(err);
        }
  ...
  describe('#_normalizeAddressArg', function() {
    it('will turn single address into array', function() {
      var widecoind = new BitcoinService(baseConfig);
      var args = widecoind._normalizeAddressArg('address');
      args.should.deep.equal(['address']);
    });
    it('will keep an array as an array', function() {
      var widecoind = new BitcoinService(baseConfig);
      var args = widecoind._normalizeAddressArg(['address', 'address']);
  ...
  describe('#getAddressBalance', function() {
    it('will give rpc error', function(done) {
      ...
      var address = 'LWx1jmpcMAQMXou6pbLyfMU7zETnbhy3bo';
      var options = {};
      widecoind.getAddressBalance(address, options, function(err) {
        err.should.be.instanceof(Error);
        done();
      });
    });
    it('will give balance', function(done) {
      ...
      var address = 'LWx1jmpcMAQMXou6pbLyfMU7zETnbhy3bo';
      var options = {};
      widecoind.getAddressBalance(address, options, function(err, data) {
        if (err) {
          return done(err);
        }
        data.balance.should.equal(10000);
        data.received.should.equal(100000);
        widecoind.getAddressBalance(address, options, function(err, data2) {
          if (err) {
            return done(err);
          }
  ...
  describe('#getAddressUnspentOutputs', function() {
    it('will give rpc error', function(done) {
      ...
      var address = 'LWx1jmpcMAQMXou6pbLyfMU7zETnbhy3bo';
      widecoind.getAddressUnspentOutputs(address, options, function(err) {
        should.exist(err);
        err.should.be.instanceof(errors.RPCError);
        done();
      });
    ...
    it('will give results from client getaddressutxos', function(done) {
      ...
      var address = 'LWx1jmpcMAQMXou6pbLyfMU7zETnbhy3bo';
      widecoind.getAddressUnspentOutputs(address, options, function(err, utxos) {
        if (err) {
          return done(err);
        }
  ...
  describe('#_getBalanceFromMempool', function() {
    it('will sum satoshis', function() {
      ...
      var sum = widecoind._getBalanceFromMempool(deltas);
  ...
  describe('#_getTxidsFromMempool', function() {
    it('will filter to txids', function() {
      ...
      var txids = widecoind._getTxidsFromMempool(deltas);
  ...
  describe('#_getHeightRangeQuery', function() {
    it('will detect range query', function() {
      ...
      var rangeQuery = widecoind._getHeightRangeQuery(options);
    ...
    it('will get range properties', function() {
      ...
      var clone = {};
      widecoind._getHeightRangeQuery(options, clone);
    ...
    it('will throw error with invalid range', function() {
      ...
      (function() {
        widecoind._getHeightRangeQuery(options);
  ...
  describe('#getAddressTxids', function() {
    it('will give error from _getHeightRangeQuery', function(done) {
      var widecoind = new BitcoinService(baseConfig);
      widecoind._getHeightRangeQuery = sinon.stub().throws(new Error('test'));
      widecoind.getAddressTxids('address', {}, function(err) {
        err.should.be.instanceOf(Error);
        err.message.should.equal('test');
        done();
      });
    ...
    it('will give rpc error from mempool query', function() {
      ...
      var options = {};
      var address = 'LWx1jmpcMAQMXou6pbLyfMU7zETnbhy3bo';
      widecoind.getAddressTxids(address, options, function(err) {
        should.exist(err);
        err.should.be.instanceof(errors.RPCError);
      });
    });
    ...
    it('will get txid results', function(done) {
      ...
      var address = 'LWx1jmpcMAQMXou6pbLyfMU7zETnbhy3bo';
      widecoind.getAddressTxids(address, options, function(err, txids) {
        if (err) {
          return done(err);
        }
    ...
    it('will get txid results from cache and live mempool', function(done) {
      ...
      var address = 'LWx1jmpcMAQMXou6pbLyfMU7zETnbhy3bo';
      widecoind.getAddressTxids(address, {queryMempool: false}, function(err, txids) {
        if (err) {
          return done(err);
        }
        getAddressTxids.callCount.should.equal(1);
        ...
        widecoind.getAddressTxids(address, {queryMempool: true}, function(err, txids) {
          if (err) {
            return done(err);
          }
          getAddressTxids.callCount.should.equal(1);
          ...
          widecoind.getAddressTxids(address, {queryMempoolOnly: true}, function(err, txids) {
            if (err) {
              return done(err);
            }
            getAddressTxids.callCount.should.equal(1);
  ...
  describe('#_getConfirmationDetail', function() {
    ...
    it('should get 0 confirmation', function() {
      ...
      var widecoind = new BitcoinService(baseConfig);
      widecoind.height = 10;
      var confirmations = widecoind._getConfirmationsDetail(tx);
  ...
  describe('#_getAddressDetailsForInput', function() {
    it('will return if missing an address', function() {
      var widecoind = new BitcoinService(baseConfig);
      var result = {};
      widecoind._getAddressDetailsForInput({}, 0, result, []);
    ...
    it('will only add address if it matches', function() {
      var widecoind = new BitcoinService(baseConfig);
      var result = {};
      widecoind._getAddressDetailsForInput({
        address: 'address1'
      }, 0, result, ['address2']);
      should.not.exist(result.addresses);
      should.not.exist(result.satoshis);
  ...
  describe('#_getAddressDetailsForOutput', function() {
    it('will return if missing an address', function() {
      var widecoind = new BitcoinService(baseConfig);
      var result = {};
      widecoind._getAddressDetailsForOutput({}, 0, result, []);
      should.not.exist(result.addresses);
      should.not.exist(result.satoshis);
    });
    it('will only add address if it matches', function() {
      var widecoind = new BitcoinService(baseConfig);
      var result = {};
      widecoind._getAddressDetailsForOutput({
        address: 'address1'
      }, 0, result, ['address2']);
      should.not.exist(result.addresses);
      should.not.exist(result.satoshis);
  ...
  describe('#_getAddressDetailsForTransaction', function() {
    it('will calculate details for the transaction', function(done) {
      ...
      var details = widecoind._getAddressDetailsForTransaction(tx, addresses);
  ...
  describe('#_getAddressDetailedTransaction', function() {
    it('will get detailed transaction info', function(done) {
      ...
      var widecoind = new BitcoinService(baseConfig);
      widecoind.getDetailedTransaction = sinon.stub().callsArgWith(1, null, tx);
      widecoind.height = 300;
      var addresses = {};
      widecoind._getAddressDetailsForTransaction = sinon.stub().returns({
        addresses: addresses,
        satoshis: 1000,
      });
      widecoind._getAddressDetailedTransaction(txid, {}, function(err, details) {
        if (err) {
          return done(err);
        }
    ...
    it('give error from getDetailedTransaction', function(done) {
      var txid = '46f24e0c274fc07708b781963576c4c5d5625d926dbb0a17fa865dcd9fe58ea0';
      var widecoind = new BitcoinService(baseConfig);
      widecoind.getDetailedTransaction = sinon.stub().callsArgWith(1, new Error('test'));
      widecoind._getAddressDetailedTransaction(txid, {}, function(err) {
        err.should.be.instanceof(Error);
        done();
      });
    });
  ...
  describe('#_getAddressStrings', function() {
    it('will get address strings from bitcore addresses', function() {
      ...
      var widecoind = new BitcoinService(baseConfig);
      var strings = widecoind._getAddressStrings(addresses);
    ...
    it('will give error with unknown', function() {
      ...
      var widecoind = new BitcoinService(baseConfig);
      (function() {
        widecoind._getAddressStrings(addresses);
  ...
  describe('#getAddressHistory', function() {
    ...
    it('will give error with "from" and "to" range that exceeds max size', function(done) {
      var widecoind = new BitcoinService(baseConfig);
      widecoind.getAddressHistory(address, {from: 0, to: 51}, function(err) {
        should.exist(err);
    ...
    it('will give error with "from" and "to" order is reversed', function(done) {
      var widecoind = new BitcoinService(baseConfig);
      widecoind.getAddressTxids = sinon.stub().callsArgWith(2, null, []);
      widecoind.getAddressHistory(address, {from: 51, to: 0}, function(err) {
        should.exist(err);
        err.message.match(/^\"from/);
        done();
      });
    ...
    it('will give error from _getAddressDetailedTransaction', function(done) {
      var widecoind = new BitcoinService(baseConfig);
      widecoind.getAddressTxids = sinon.stub().callsArgWith(2, null, ['txid']);
      widecoind._getAddressDetailedTransaction = sinon.stub().callsArgWith(2, new Error('test'));
      widecoind.getAddressHistory(address, {}, function(err) {
        should.exist(err);
        err.message.should.equal('test');
        done();
      });
    ...
    it('will give an error if length of addresses is too long', function(done) {
      ...
      var widecoind = new BitcoinService(baseConfig);
      widecoind.maxAddressesQuery = 100;
      widecoind.getAddressHistory(addresses, {}, function(err) {
        should.exist(err);
    ...
    it('give error from getAddressTxids', function(done) {
      var widecoind = new BitcoinService(baseConfig);
      widecoind.getAddressTxids = sinon.stub().callsArgWith(2, new Error('test'));
      widecoind.getAddressHistory('address', {}, function(err) {
        should.exist(err);
        err.should.be.instanceof(Error);
        err.message.should.equal('test');
    ...
    it('will paginate', function(done) {
      var widecoind = new BitcoinService(baseConfig);
      widecoind._getAddressDetailedTransaction = function(txid, options, callback) {
        callback(null, txid);
      };
      var txids = ['one', 'two', 'three', 'four'];
      widecoind.getAddressTxids = sinon.stub().callsArgWith(2, null, txids);
      widecoind.getAddressHistory('address', {from: 1, to: 3}, function(err, data) {
        if (err) {
          return done(err);
        }
  ...
  describe('#getAddressSummary', function() {
    ...
    it('will handle error from getAddressTxids', function(done) {
      ...
      widecoind.getAddressTxids = sinon.stub().callsArgWith(2, new Error('test'));
      widecoind.getAddressBalance = sinon.stub().callsArgWith(2, null, {});
      var address = '';
      var options = {};
      widecoind.getAddressSummary(address, options, function(err) {
        should.exist(err);
        err.should.be.instanceof(Error);
        err.message.should.equal('test');
        done();
    ...
    it('will handle error from getAddressBalance', function(done) {
      ...
      widecoind.getAddressTxids = sinon.stub().callsArgWith(2, null, {});
      widecoind.getAddressBalance = sinon.stub().callsArgWith(2, new Error('test'), {});
      var address = '';
      var options = {};
      widecoind.getAddressSummary(address, options, function(err) {
        should.exist(err);
    ...
    it('should set all properties', function(done) {
      ...
      sinon.spy(widecoind, '_paginateTxids');
      widecoind.getAddressTxids = sinon.stub().callsArgWith(2, null, [txid1, txid2, txid3]);
      widecoind.getAddressBalance = sinon.stub().callsArgWith(2, null, {
        received: 30 * 1e8,
        balance: 20 * 1e8
      });
      ...
      var options = {};
      widecoind.getAddressSummary(address, options, function(err, summary) {
        widecoind._paginateTxids.callCount.should.equal(1);
        widecoind._paginateTxids.args[0][1].should.equal(0);
        widecoind._paginateTxids.args[0][2].should.equal(1000);
    ...
    it('will get from cache with noTxList', function(done) {
      ...
      widecoind.getAddressSummary(address, options, function(err, summary) {
        ...
        widecoind.getAddressTxids.callCount.should.equal(1);
        widecoind.getAddressBalance.callCount.should.equal(1);
        widecoind.getAddressSummary(address, options, function(err, summary) {
          checkSummary(summary);
          widecoind.getAddressTxids.callCount.should.equal(1);
          widecoind.getAddressBalance.callCount.should.equal(1);
    ...
    it('will skip querying the mempool with queryMempool set to false', function(done) {
      ...
      widecoind.getAddressSummary(address, options, function() {
        getAddressMempool.callCount.should.equal(0);
        done();
      });
    });
    it('will give error from _paginateTxids', function(done) {
      ...
      widecoind._paginateTxids = sinon.stub().throws(new Error('test'));
      var address = '3NbU8XzUgKyuCgYgZEKsBtUvkTm2r7Xgwj';
      var options = {
        queryMempool: false
      };
  ...
  describe('#getRawBlock', function() {
    ...
    it('will give rcp error from client getblockhash', function(done) {
      ...
      widecoind.getRawBlock(10, function(err) {
        should.exist(err);
        err.should.be.instanceof(errors.RPCError);
        done();
      });
    ...
    it('will give rcp error from client getblock', function(done) {
      ...
      widecoind.getRawBlock(blockhash, function(err) {
        should.exist(err);
        err.should.be.instanceof(errors.RPCError);
        done();
      });
    ...
    it('will try all nodes for getblock', function(done) {
      ...
      widecoind.getRawBlock(blockhash, function(err, buffer) {
        if (err) {
          return done(err);
        }
        buffer.should.be.instanceof(Buffer);
    ...
    it('will get block by height', function(done) {
      ...
      widecoind.getRawBlock(0, function(err, buffer) {
        if (err) {
          return done(err);
        }
        buffer.should.be.instanceof(Buffer);
  ...
  describe('#getBlock', function() {
    ...
    it('will give an rpc error from client getblock', function(done) {
      ...
      widecoind.getBlock(0, function(err) {
        err.should.be.instanceof(Error);
        done();
      });
    });
    ...
    it('will getblock as bitcore object from height', function(done) {
      ...
      widecoind.getBlock(0, function(err, block) {
        should.not.exist(err);
    ...
    it('will getblock as bitcore object', function(done) {
      ...
      widecoind.getBlock('00000000050a6d07f583beba2d803296eb1e9d4980c4a20f206c584e89a4f02b', function(err, block) {
        should.not.exist(err);
    ...
    it('will get block from cache', function(done) {
      ...
      widecoind.getBlock(hash, function(err, block) {
        should.not.exist(err);
        getBlockHash.callCount.should.equal(0);
        getBlock.callCount.should.equal(1);
        block.should.be.instanceof(bitcore.Block);
  ...
  describe('#getBlockHashesByTimestamp', function() {
    it('should give an rpc error', function(done) {
      ...
      widecoind.getBlockHashesByTimestamp(1441911000, 1441914000, function(err, hashes) {
        should.exist(err);
    ...
    it('should get the correct block hashes', function(done) {
      ...
      widecoind.getBlockHashesByTimestamp(1441914000, 1441911000, function(err, hashes) {
        should.not.exist(err);
  ...
  describe('#getBlockHeader', function() {
    ...
    it('will give error from getBlockHash', function() {
      ...
      widecoind.getBlockHeader(10, function(err) {
        err.should.be.instanceof(Error);
      });
    });
    it('it will give rpc error from client getblockheader', function() {
      ...
      widecoind.getBlockHeader(blockhash, function(err) {
        err.should.be.instanceof(Error);
      });
    });
    it('it will give rpc error from client getblockhash', function() {
      ...
      widecoind.getBlockHeader(0, function(err) {
        err.should.be.instanceof(Error);
      });
    });
    it('will give result from client getblockheader (from height)', function() {
      ...
      widecoind.getBlockHeader(0, function(err, blockHeader) {
        should.not.exist(err);
    ...
    it('will give result from client getblockheader (from hash)', function() {
      ...
      widecoind.getBlockHeader(blockhash, function(err, blockHeader) {
        should.not.exist(err);
        getBlockHash.callCount.should.equal(0);
        blockHeader.should.deep.equal(result);
      });
  ...
  describe('#_maybeGetBlockHash', function() {
    it('will not get block hash with an address', function(done) {
      ...
      widecoind._maybeGetBlockHash('2N2JD6wb56AfK4tfmM6PwdVmoYk2dCKf4Br', function(err, hash) {
        if (err) {
          return done(err);
        }
        getBlockHash.callCount.should.equal(0);
    ...
    it('will not get block hash with non zero-nine numeric string', function(done) {
      ...
      widecoind._maybeGetBlockHash('109a', function(err, hash) {
        if (err) {
          return done(err);
        }
        getBlockHash.callCount.should.equal(0);
    ...
    it('will get the block hash if argument is a number', function(done) {
      ...
      widecoind._maybeGetBlockHash(10, function(err, hash) {
        if (err) {
          return done(err);
        }
  ...
  describe('#getBlockOverview', function() {
    ...
    it('will handle error from maybeGetBlockHash', function(done) {
      var widecoind = new BitcoinService(baseConfig);
      widecoind._maybeGetBlockHash = sinon.stub().callsArgWith(1, new Error('test'));
      widecoind.getBlockOverview(blockhash, function(err) {
        err.should.be.instanceOf(Error);
        done();
      });
    });
    ...
    it('will give expected result', function(done) {
      ...
      widecoind.getBlockOverview(blockhash, function(err, blockOverview) {
        if (err) {
          return done(err);
        }
  ...
  describe('#estimateFee', function() {
    it('will give rpc error', function(done) {
      ...
      widecoind.estimateFee(1, function(err) {
        should.exist(err);
        err.should.be.an.instanceof(errors.RPCError);
        done();
      });
    ...
    it('will call client estimateFee and give result', function(done) {
      ...
      widecoind.estimateFee(1, function(err, feesPerKb) {
        if (err) {
          return done(err);
        }
  ...
  describe('#sendTransaction', function(done) {
    ...
    it('will give rpc error', function() {
      ...
      widecoind.sendTransaction(txhex, function(err) {
        should.exist(err);
        err.should.be.an.instanceof(errors.RPCError);
      });
    });
    it('will send to client and get hash', function() {
      ...
      widecoind.sendTransaction(txhex, function(err, hash) {
        if (err) {
          return done(err);
        }
    ...
    it('will send to client with absurd fees and get hash', function() {
      ...
      widecoind.sendTransaction(txhex, {allowAbsurdFees: true}, function(err, hash) {
        if (err) {
          return done(err);
        }
        hash.should.equal(tx.hash);
    ...
    it('missing callback will throw error', function() {
      ...
      var transaction = bitcore.Transaction();
      (function() {
        widecoind.sendTransaction(transaction);
      }).should.throw(Error);
    });
  });
  ...
  describe('#getRawTransaction', function() {
    it('will give rpc error', function(done) {
      ...
      widecoind.getRawTransaction('txid', function(err) {
        should.exist(err);
        err.should.be.an.instanceof(errors.RPCError);
        done();
      });
    ...
    it('will try all nodes', function(done) {
      ...
      widecoind.getRawTransaction('txid', function(err, tx) {
        if (err) {
          return done(err);
        }
        should.exist(tx);
  ...
  describe('#getTransaction', function() {
    it('will give rpc error', function(done) {
      ...
      widecoind.getTransaction('txid', function(err) {
        should.exist(err);
        err.should.be.an.instanceof(errors.RPCError);
        done();
      });
    ...
    it('will try all nodes', function(done) {
      ...
      widecoind.getTransaction('txid', function(err, tx) {
        if (err) {
          return done(err);
        }
        should.exist(tx);
  ...
  describe('#getDetailedTransaction', function() {
    ...
    it('should give a transaction with height and timestamp', function(done) {
      ...
      widecoind.getDetailedTransaction(txid, function(err) {
        should.exist(err);
        err.should.be.instanceof(errors.RPCError);
        done();
      });
    ...
    it('should give a transaction with all properties', function(done) {
      ...
      widecoind.getDetailedTransaction(txid, function(err, tx) {
        if (err) {
          return done(err);
        }
        checkTx(tx);
  ...
  describe('#getBestBlockHash', function() {
    it('will give rpc error', function(done) {
      ...
      widecoind.getBestBlockHash(function(err) {
        should.exist(err);
        err.should.be.an.instanceof(errors.RPCError);
        done();
      });
    ...
    it('will call client getInfo and give result', function(done) {
      ...
      widecoind.getBestBlockHash(function(err, hash) {
        if (err) {
          return done(err);
        }
        should.exist(hash);
  ...
  describe('#getSpentInfo', function() {
    it('will give rpc error', function(done) {
      ...
      widecoind.getSpentInfo({}, function(err) {
        should.exist(err);
        err.should.be.an.instanceof(errors.RPCError);
        done();
      });
    ...
    it('will empty object when not found', function(done) {
      ...
      widecoind.getSpentInfo({}, function(err, info) {
        should.not.exist(err);
  ...
  describe('#getInfo', function() {
    it('will give rpc error', function(done) {
      ...
      widecoind.getInfo(function(err) {
        should.exist(err);
        err.should.be.an.instanceof(errors.RPCError);
        done();
      });
    ...
    it('will call client getInfo and give result', function(done) {
      var widecoind = new BitcoinService(baseConfig);
      widecoind.node.getNetworkName = sinon.stub().returns('testnet');
      ...
      widecoind.getInfo(function(err, info) {
        if (err) {
          return done(err);
        }
        should.exist(info);
  ...
  describe('#generateBlock', function() {
    it('will give rpc error', function(done) {
      ...
      widecoind.generateBlock(10, function(err) {
        should.exist(err);
        err.should.be.an.instanceof(errors.RPCError);
        done();
      });
    ...
    it('will call client generate and give result', function(done) {
      ...
      widecoind.generateBlock(10, function(err, hashes) {
        if (err) {
          return done(err);
        }
  ...
  describe('#stop', function() {
    ...
    it('will exit spawned process', function(done) {
      var widecoind = new BitcoinService(baseConfig);
      widecoind.spawn = {};
      widecoind.spawn.process = new EventEmitter();
      widecoind.spawn.process.kill = sinon.stub();
      widecoind.stop(done);
      widecoind.spawn.process.kill.callCount.should.equal(1);
      widecoind.spawn.process.kill.args[0][0].should.equal('SIGINT');
      widecoind.spawn.process.emit('exit', 0);
    });
    it('will give error with non-zero exit status code', function(done) {
      ...
      widecoind.spawn.process.kill.callCount.should.equal(1);
      widecoind.spawn.process.kill.args[0][0].should.equal('SIGINT');
      widecoind.spawn.process.emit('exit', 1);
    });
    it('will stop after timeout', function(done) {
      var widecoind = new BitcoinService(baseConfig);
      widecoind.shutdownTimeout = 300;
      widecoind.spawn = {};
      widecoind.spawn.process = new EventEmitter();
      widecoind.spawn.process.kill = sinon.stub();
      widecoind.stop(function(err) {
